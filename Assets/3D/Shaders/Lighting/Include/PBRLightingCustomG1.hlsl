...
void OrenNayar_float(float3 normalWS, float3 lightDirWS, float3 viewDirWS, float roughness, out float diffuse)
{
    ...
    float A = 1.0f - 0.5f * (roughnessSq/(roughnessSq + 0.57));
    float B = 0.45f * (roughnessSq / (roughnessSq + 0.09));
...
void ShadeAdditionalLightsPBR_float(float2 uvSS, float3 normalWS, float3 viewDirWS, float3 positionWS, float smoothness, out float3 diffuse, out float3 specular)
{
    diffuse = 0;
    specular = 0;
